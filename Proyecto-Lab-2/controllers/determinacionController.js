  ...
  try{
    const resultados = await ValorRef.findAll({
      ...
      attributes: [
        ...
        [sequelize.col('determinacion.determinacion.examen.resultado'), 'resultado'],
        [sequelize.col('determinacion.determinacion.examen.descripcion'), 'descripcionB'],
        'val_max',
        'val_min',
        'sexo',
        'rango_edad'
...
const orden = await Orden.findOne({
  ...
  where: {
   ...
   // res.json({ resultados:resultadosFiltrados, orden});
 res.render('vistaExaYdeter', { resultados: resultadosFiltrados,orden,rol,nombre,apellido });
  } catch (error) {
    throw new Error(`Error al realizar la consulta: ${error.message}`);
...
const resultadosFiltrados = resultados.filter(item => item.dataValues.Valor !== null);
  ...
  //res.json(resultadosFiltrados);
    res.render('vistaRegistrarValor', { resultados: resultadosFiltrados,nombre, apellido });
  } catch (error) {
    throw new Error(`Error al realizar la consulta: ${error.message}`);
