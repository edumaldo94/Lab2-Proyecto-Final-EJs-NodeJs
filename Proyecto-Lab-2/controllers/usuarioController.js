...
const btnAlta= async (req, res)=>{
  ...
  const btnBaja= async (req, res)=>{
    ...
    const modificarRol= async (req, res)=>{
      ...
      const obtenerPersonasSinUsuario = async (req, res) => {
        try {
          const query = `
            SELECT persona.dni, persona.nombre, persona.apellido, persona.email
            FROM persona
            LEFT JOIN usuario ON persona.dni = usuario.dni
            WHERE usuario.id_usuario IS NULL;
          `;
          const personasSinUsuario = await sequelize.query(query, { type: sequelize.QueryTypes.SELECT });
          console.log("Personas sin usuario:", personasSinUsuario);
          return personasSinUsuario;
        } catch (error) {
          throw new Error('Error al obtener personas sin usuario: ' + error.message);
        }
      };
