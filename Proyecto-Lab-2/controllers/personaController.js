...
const create = async (req, res) => {
  const { nombre, apellido, dni, fecha_nacimient, genero, embarazada, provincia, ciudad, direccion, tel, email } = req.body;
...
const update = async (req, res) => {
  ...
  const { nombre, apellido, dni, fecha_nacimient, genero, provincia,ciudad,direccion, tel, email } = req.body; // Nuevos datos
  ...
  try {
    ...
    if (updatedPersona[0] === 1) {
      ...
      res.status(404).json({ error: 'Persona no encontrada XXXX' });
    }
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
const updateDni = async (req, res) => {
  ...
  const { nombre, apellido, fecha_nacimient, genero, provincia,ciudad,direccion, tel, email } = req.body; // Nuevos datos
...
const renderDni = async (req, res) => {
  ...
  try {
    ...
    if (!persona) {
      ...
      return res.render("createPaciente",{ rol })
...
if (persona.id_persona != null) {  
    res.render('panelPaciente',{ persona, ordenes,examen,resOrd2,cambioEstado, resOrd,ultimoCambioOrden, rol })
     ...
     console.log("hoasdasdasdasdasdas")
      res.send(false)
    }
  } catch (error) {
    res.status(500).json({ error: error.message });
...
const ultimoCambioOrden = obtenerMenorIdEstado(resOrd2);
    ...
    if (persona) { 
      res.render('panelPaciente',{ persona, ordenes,examen ,resOrd,resOrd2,ultimoCambioOrden,rol})
...
const buscarPorDniUser = async (dni) => {
  try {
    const persona = await Persona.findOne({
      where: {
        dni: dni,
    ...
    if (persona) {
      ...
      return null; // Retorna null si no se encuentra ninguna persona con ese DNI
    }
  } catch (error) {
    console.error('Error al buscar la persona:', error);
...
const renderDniPaciente = async (dni, rol, res) => {
 // const { dni } = req.body;
 // const { rol } = req.params;
  try {
    const persona = await Persona.findOne({
...
if (persona) {  
   res.render('page-paciente',{ persona, ordenes,examen,resOrd2,cambioEstado, resOrd,ultimoCambioOrden, rol })
                         //res.json({ persona, ordenes,examen,resOrd2,cambioEstado, resOrd,ultimoCambioOrden, rol });
