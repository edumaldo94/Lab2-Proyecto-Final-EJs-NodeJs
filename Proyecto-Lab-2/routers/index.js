...
router.get("/page-Gerente",authMiddleware.requireAuth, async(req,res)=>{
  ...
  router.get('/page-Administrador', authMiddleware.requireAuth,(req, res) => {
    ...
    res.render("page-administrativo",{rol:'Administrador',nombre,apellido})	
    

  });
  
  router.get("/page-Paciente/:dni/:rol",authMiddleware.requireAuth,(req,res)=>{
    const { rol } = req.params;
    const { dni } = req.params;
    console.log(dni+rol);
    // Llama a la función renderDniPaciente y pásale los parámetros
    personaController.renderDniPaciente(dni, rol, res);
  });
  ...
  router.get("/page-Tecnico",authMiddleware.requireAuth, async(req,res)=>{
    ...
    try {
      ...
      res.render("page-Tecnico", { rol: 'Tecnico',nombre,apellido, ordenes, ordenes2 });
  ...
  router.get("/page-Bioquimico",authMiddleware.requireAuth,(req,res)=>{
    ...
    res.render("page-Bioquimico",{rol:'Bioquimico',nombre,apellido,ordenes})	
  })
  })
  //--------------------------------------------------------------------
...
router.get('/registrar-valores/:id/:genero/:rol',authMiddleware.requireAuth,async (req,res)=>{
  ...
  try {
    await examenController.getForReg(id, genero, (examen, determinacion, valor_Ref) => {
     ...
     res.render('registrar_valores',{examen, determinacion, valor_Ref, persona,rol,nombre, apellido})
        /*
     res.json({
          ok: true,
          persona: persona,
...
router.get('/registrar-valoresPreInfo/:id/:genero',authMiddleware.requireAuth,async (req,res)=>{
  ...
  try {
    await examenController.getPreInfo(id, genero, (examen, determinacion, valor_Ref, registroDeValor) => {
     ...
     res.render('registroValorPreInfo',{examen, determinacion, valor_Ref, persona, registroDeValor,nombre, apellido})
        /*
     res.json({
          ok: true,
          persona: persona,
